...
The `SubscribeToAll` method is identical to the `SubscribeToStream` method, except that you don't need to specify a stream name.
...
```java
client.deleteToStream("stream-name", "subscription-group");
```
